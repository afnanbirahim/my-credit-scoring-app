...
# Repayment ratio (user-estimated)
repayment_ratio = st.slider("Estimated Repayment Ratio (0.0–1.0)", min_value=0.0, max_value=1.0, step=0.05, value=0.75)

# Threshold to classify eligible/ineligible
threshold = st.slider("Approval Threshold", min_value=0.3, max_value=0.9, step=0.01, value=0.65)

# Feature engineering
debt_to_income = loan_amount / (income + 1)
savings_to_income = savings / (income + 1)

# Core input dictionary
user_input = {
...
# Fill any missing features (e.g., one-hot encoded) with 0.0
...
if st.button("Check Eligibility"):
    ...
    st.subheader("Scaled Input (Used by Model)")
